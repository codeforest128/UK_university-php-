...
if (isset($_POST['login'])) {
    ...
    if (DB::query('SELECT email FROM clients WHERE email=:username', array(':username' => $username))) {

        if (password_verify($password, DB::query('SELECT password FROM clients WHERE email=:username', array(':username' => $username))[0]['password'])) {
            $cstrong = True;
            $token = bin2hex(openssl_random_pseudo_bytes(64, $cstrong));

            $client_id = DB::query('SELECT id FROM clients WHERE email=:username', array(':username' => $username))[0]['id'];
...
<head>
    ...
    <title>Client Login - Oxford Careers</title>
...
<body>
        ...
        <div class="container"><a class="navbar-brand logo" href="../index.php"><img src="../logo.png" style="height:60px;width:160px;"></a><button class="navbar-toggler" data-toggle="collapse" data-target="#navcol-1"><span class="sr-only">Toggle navigation</span><span class="navbar-toggler-icon"></span></button>
            <div class="collapse navbar-collapse" id="navcol-1">
                <ul class="nav navbar-nav ml-auto">
                    ...
                    <li class="nav-item" role="presentation"><a class="nav-link text-primary" style="width:150px" href="login.php">Login</a></li>
                </ul>
            </div>
        </div>
    </nav>
    ...
    <div class="login-clean">
        <form method="post">
            ...
            <label style="margin-left:25%;font-size:18px;">Client Login</label>
            <div class="form-group"><input class="form-control" type="email" name="email" placeholder="Email"></div>
            <div class="form-group"><input class="form-control" type="password" name="password" placeholder="Password"></div>
            <div class="form-group"><label style="color:red"><?php echo $message ?></label></div>
            <div class="form-group"><input class="btn btn-primary btn-block" type="submit" value="Login as Client" name="login" style="background-color: rgb(48,23,119);"></div><a href="forgot-pass.php" class="forgot">Forgot your email or password?</a>
            <!-----a href="signup.php" class="forgot">Don't have an Account?<strong> Sign Up</strong> </a--------->
        </form>
    </div>
